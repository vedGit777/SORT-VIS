//Links are attached for
//disabled
//parseInt...here helps us get height of the bar
//split...to get the bar id

let bars = [];
const def = "#AF0404", chng = "#AF0404", finished = "#AF0404", selected = "#cfa7c9";
...
async function setup() {
	...
	if (bars.length != parseInt(b.value)) {  //parseInt is a JS method -> https://www.w3schools.com/jsref/jsref_parseint.asp
...
function Disable_The_Input() {
	...
	for (let i = 0; i < x.length; i++)
		x[i].disabled = true;  //disabled is a js property -> check this-> https://www.w3schools.com/jsref/prop_select_disabled.asp
	return parseInt(document.getElementById("delay").value); //returns the "time per operation" value ...the  millisecond one
}


function Finished_Sorting() {
	let x = document.getElementsByClassName("bar");
	for (let i = 0; i < x.length; i++)  //change color of each bar
		x[i].style.backgroundColor = finished;
	x = document.getElementsByTagName("input");
	for (let i = 0; i < x.length; i++)//disable each bar
		x[i].disabled = false;
}
...
async function SelectionSort() {
	...
	document.getElementsByClassName("comp")[0].innerHTML = "SELECTION SORT: O(n<sup>2</sup>)";
			
	for (let i = 0; i < bars.length; i++) 
	{
		let mn_ind = i; //mn_ind will be the index of min element from i+1 to n-1
		let curr_id = bars[i].split('id="')[1].split('"')[0]; //get id of current bar //split->https://www.w3schools.com/jsref/jsref_split.asp

		document.getElementById(curr_id).style.backgroundColor = selected;  //change color..since this bar is selected
		let sound = MapRange(document.getElementById(curr_id).style.height.split('%')[0], 2, 100, 500, 1000);
		
		beep(200, sound, delay) //beep sound
		...
		for (let j = i + 1; j < bars.length; j++) 
		{
			...
			let a = parseInt(bars[mn_ind].split(/[:%]/)[1]);  //got ht of bar i
			let b = parseInt(bars[j].split(/[:%]/)[1]); //height of bar j
			
			if (a > b) 
				mn_ind = j;  //changing the minimum element from i+1 to n-1
		
			await Sleep(delay / 5.0);  //basically theres a delay
		...
		let tmp = bars[mn_ind];  //swap the (ith bar) and (the minimum bar from i+1 to n-1)
		bars[mn_ind] = bars[i];
		bars[i] = tmp;

		container.innerHTML = bars.join('');
		await Sleep(2 * delay / 5.0);
		document.getElementById(curr_id).style.backgroundColor = def;  //change the color
		document.getElementById(nxt_ele).style.backgroundColor = def;
	}
	Finished_Sorting();
}
...
async function BubbleSort() {
	let delay = Disable_The_Input();//check out the return value of Disable_The_Input
	let container = document.getElementById("container");
	
	document.getElementsByClassName("comp")[0].innerHTML = "BUBBLE SORT: O(n<sup>2</sup>)";

	for (let i = 0; i < bars.length - 1; i++) 
	{
		let has_swap = false;

		for (let j = 0; j < bars.length - i - 1; j++) //see the upper bound of j...obvio...since elements are arranged in sorted order from the end
		{
			let curr_id = bars[j].split('id="')[1].split('"')[0];
			let nxt_ele = bars[j + 1].split('id="')[1].split('"')[0];

			document.getElementById(curr_id).style.backgroundColor = selected;//change color of the selected bar

			let sound = MapRange(document.getElementById(curr_id).style.height.split('%')[0], 2, 100, 500, 1000);
			beep(200, sound, delay)//for the beep sound
...
async function InsertionSort() {
	...
	document.getElementsByClassName("comp")[0].innerHTML = "INSERTION SORT: O(n<sup>2</sup>)";
...
async function merge(l, m, r, d) {
	...
	while (i < j && j <= r) 
	{
		let curr_id = bars[j].split('id="')[1].split('"')[0]; //getting ids
		let nxt_ele = bars[i].split('id="')[1].split('"')[0];
		document.getElementById(curr_id).style.backgroundColor = selected;  //changing colors
		document.getElementById(nxt_ele).style.backgroundColor = chng;
		let a = parseInt(bars[j].split(/[:%]/)[1]);  //getting heights
		...
		document.getElementsByClassName("comp")[0].innerHTML = "MERGE SORT: O(nlog(n))";
...
async function mergeSort(l, r, d) {
	if (l < r) 
	{
		...
		await mergeSort(l, m, d); //sort left half
		await mergeSort(m + 1, r, d); //sort right half
		await merge(l, m, r, d); //merge
	}
}


async function MergeSort() {  //main function
	let delay = Disable_The_Input();
...
async function Partition(l, r, d) {
	...
	document.getElementsByClassName("comp")[0].innerHTML = "QUICK SORT: O(nlog(n))";
	...
	for (j = l; j < r; j++) 
	{
		...
		if (a < b) //when bar r is taller
		{
			...
			let temp = bars[i]; //swapping heights of i and j bar
			bars[i] = bars[j];
			bars[j] = temp;
	...
	let temp = bars[i + 1]; //putting the bar r to its correct posn
	bars[i + 1] = bars[r];
	bars[r] = temp;
...
async function quickSort(l, r, d) {
	if (l < r) 
	{
		let p = await Partition(l, r, d);  //partition
		await quickSort(l, p - 1, d);  //on left half
		await quickSort(p + 1, r, d);  //on right half
	}
}


async function QuickSort() {  //main function
	let delay = Disable_The_Input();
...
async function Heapfiy(n, i, d) {
	...
	document.getElementsByClassName("comp")[0].innerHTML = "HEAP SORT: O(nlog(n))";
